    ...
    <div className="fixed inset-0 z-[60] flex flex-col bg-canvas">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-card px-4 py-3 sm:px-6">
        <div className="flex min-w-0 items-center gap-3">
          <Button variant="ghost" size="icon" onClick={closeSafely} aria-label={`Back to ${draft.group === "invites" ? "Automated Invites" : "campaigns"}`}><ArrowLeft size={17} /></Button>
          <div className="min-w-0"><p className="text-[10.5px] font-medium text-muted-foreground">← Automated Invites</p><h2 className="truncate text-[17px] font-semibold text-card-foreground">{draft.name}</h2><p className="truncate text-[11.5px] text-muted-foreground">{draft.timing} · {STRATEGY_LABEL[draft.strategy]}</p></div>
        </div>
        <div className="flex items-center gap-2"><span className={`text-[11.5px] ${dirty ? "text-brand" : "text-muted-foreground"}`}>{dirty ? "Unsaved changes" : "All changes saved"}</span><Button variant="brand" size="sm" disabled={!dirty} onClick={requestSave}><Check size={14} />Save changes</Button></div>
      </header>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6"><div className="mx-auto max-w-6xl rounded-lg border border-border bg-card p-4 shadow-card sm:p-6">{activeChannel === "text" ? <TextEditor value={variant.text} onChange={(text) => setVariant({ ...variant, text }, "text")} /> : <EmailEditor value={variant.email} customized={variant.customization.email} onChange={(email) => setVariant({ ...variant, email }, "email")} />}</div></div>
